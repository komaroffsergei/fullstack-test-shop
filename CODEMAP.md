...
| API                   | validation, logs, Swagger, static Angular, listen | [main.ts:14](https://github.com/komaroffsergei/fullstack-test-shop/blob/main/apps/api/src/main.ts#L14)                                                                                                       | [main.ts:14](docs/offline/index.html#file-apps-api-src-main-ts-L14)                                                                      |
...
| Local acceptance      | 13 DB-backed scenarios                            | [race main:519](https://github.com/komaroffsergei/fullstack-test-shop/blob/main/tests/race/run.ts#L519)                                                                                                      | [race main:519](docs/offline/index.html#file-tests-race-run-ts-L519)                                                                     |
| Production acceptance | 9 HTTPS scenarios                                 | [production main:304](https://github.com/komaroffsergei/fullstack-test-shop/blob/main/tests/production/run.ts#L304)                                                                                          | [production main:304](docs/offline/index.html#file-tests-production-run-ts-L304)                                                         |
...
| Создание заказа             | product/promo lock/order snapshot/redemption выполняются в одной короткой transaction; P2002 читает победителя | Касса запирает купон, записывает заказ и не даёт второму кассиру сделать копию  | [createOrder():64](https://github.com/komaroffsergei/fullstack-test-shop/blob/main/apps/api/src/shop.service.ts#L64)                                                 | [createOrder():64](docs/offline/index.html#file-apps-api-src-shop-service-ts-L64)                                                 |
| Promo concurrency           | `SELECT … FOR UPDATE` сериализует check-and-increment `used_count`                                             | К одному листу купонов подходят по одному и ставят отметку                      | [promo lock:82](https://github.com/komaroffsergei/fullstack-test-shop/blob/main/apps/api/src/shop.service.ts#L82)                                                    | [promo lock:82](docs/offline/index.html#file-apps-api-src-shop-service-ts-L82)                                                    |
| Durable webhook             | INSERT event выполняется до `200`; event ID UNIQUE превращает replay в no-op                                   | Письмо сначала кладут в несгораемый ящик, потом говорят «получили»              | [acceptWebhook():186](https://github.com/komaroffsergei/fullstack-test-shop/blob/main/apps/api/src/shop.service.ts#L186)                                             | [acceptWebhook():186](docs/offline/index.html#file-apps-api-src-shop-service-ts-L186)                                             |
| Раннее событие              | inbox UUID намеренно без FK; worker JOIN не claim-ит событие до order                                          | Письмо ждёт коробку с таким номером и никуда не исчезает                        | [processPaymentEvent():67](https://github.com/komaroffsergei/fullstack-test-shop/blob/main/apps/worker/src/worker.service.ts#L67)                                    | [processPaymentEvent():67](docs/offline/index.html#file-apps-worker-src-worker-service-ts-L67)                                    |
| Snapshot payment validation | amount/currency сравниваются с immutable order; mismatch → invalid без job                                     | Если в письме неправильная сумма, склад не выдаёт подарок                       | [worker:95](https://github.com/komaroffsergei/fullstack-test-shop/blob/main/apps/worker/src/worker.service.ts#L95)                                                   | [worker:95](docs/offline/index.html#file-apps-worker-src-worker-service-ts-L95)                                                   |
| Одна delivery job           | upsert опирается на UNIQUE `delivery_jobs.order_id`                                                            | На одну коробку разрешена одна рабочая карточка                                 | [worker:114](https://github.com/komaroffsergei/fullstack-test-shop/blob/main/apps/worker/src/worker.service.ts#L114)                                                 | [worker:114](docs/offline/index.html#file-apps-worker-src-worker-service-ts-L114)                                                 |
| Atomic job claim            | `UPDATE … FOR UPDATE SKIP LOCKED … RETURNING`, lease; HTTP после commit                                        | Работник быстро подписывает одну карточку и отпускает шкаф до звонка поставщику | [processDeliveryJob():153](https://github.com/komaroffsergei/fullstack-test-shop/blob/main/apps/worker/src/worker.service.ts#L153)                                   | [processDeliveryJob():153](docs/offline/index.html#file-apps-worker-src-worker-service-ts-L153)                                   |
| Timeout A                   | timeout неоднозначен; retry той же пары order/A; B запрещён                                                    | Первый курьер мог уже принести подарок, поэтому второго пока не зовём           | [timeout branch:187](https://github.com/komaroffsergei/fullstack-test-shop/blob/main/apps/worker/src/worker.service.ts#L187)                                         | [timeout branch:187](docs/offline/index.html#file-apps-worker-src-worker-service-ts-L187)                                         |
| Stable provider request     | upsert `(order, provider)` переиспользует UUID и журналирует каждую попытку                                    | Одна и та же просьба всегда имеет тот же номер                                  | [callProvider():213](https://github.com/komaroffsergei/fullstack-test-shop/blob/main/apps/worker/src/worker.service.ts#L213)                                         | [callProvider():213](docs/offline/index.html#file-apps-worker-src-worker-service-ts-L213)                                         |
...
| Final fulfillment           | order row lock + fulfillment order/code UNIQUE + status/history/job в одной transaction                        | Последняя дверь не пропустит второй ключ в ту же коробку                        | [complete():292](https://github.com/komaroffsergei/fullstack-test-shop/blob/main/apps/worker/src/worker.service.ts#L292)                                             | [complete():292](docs/offline/index.html#file-apps-worker-src-worker-service-ts-L292)                                             |
| Recovery                    | прежняя job переводится pending; provider requests не заменяются                                               | Сломанную карточку возвращают в очередь, но её номера остаются прежними         | [retryDelivery():244](https://github.com/komaroffsergei/fullstack-test-shop/blob/main/apps/api/src/shop.service.ts#L244)                                             | [retryDelivery():244](docs/offline/index.html#file-apps-api-src-shop-service-ts-L244)                                             |
| Safe reset                  | table locks закрывают check/delete race; processing job → 503                                                  | Уборку не начинают, пока работник держит карточку                               | [resetDemo():298](https://github.com/komaroffsergei/fullstack-test-shop/blob/main/apps/api/src/shop.service.ts#L298)                                                 | [resetDemo():298](docs/offline/index.html#file-apps-api-src-shop-service-ts-L298)                                                 |
...
| Public Nginx/TLS stack                | [production](docs/offline/index.html#file-tests-production-run-ts)                                                | 9 black-box сценариев, JSON report без secrets         |
