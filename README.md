...
Первый Playwright-тест проверяет все пять интерактивов через реальные clicks/hover и вычисленные CSS-стили, затем выполняет покупку. Второй ждёт сетевую готовность всех 22 изображений. Третий сверяет 12 товаров, поиск, фильтры, 4 промокода и 50 исходных ключей. Четвёртый открывает каталог при `390×844` и сравнивает `scrollWidth ≤ clientWidth`.
